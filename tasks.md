...
### 4.1 시청자 API ✅ ViewerService 6 tests + ViewerController 5 tests
- [x] GET `/api/v1/videos` - 공개 영상 목록 (페이징, 검색)
- [x] GET `/api/v1/videos/{videoId}` - 영상 재생 정보 (HLS URL + 메타데이터)
- [x] GET `/api/v1/playlists` - 공개 재생목록 목록
- [x] GET `/api/v1/playlists/{playlistId}` - 재생목록 상세 (영상 순서 포함)
- [x] 조회수 카운트 로직 (영상 재생 시 viewCount 증가)

### 4.2 HLS 비디오 플레이어
- [x] useHlsPlayer 훅 (HLS.js 초기화, 소스 로딩, 에러 처리)
- [x] VideoPlayer 컴포넌트 (재생 컨트롤, 프로그레스바, 볼륨)
- [x] HLS 미지원 브라우저 폴백 (네이티브 video 태그)

### 4.3 시청자 페이지
- [x] 영상 목록 페이지 (썸네일 그리드, 검색, 페이징)
- [x] 영상 상세 재생 페이지 (플레이어 + 영상 정보)
- [x] 재생목록 목록 페이지
- [x] 재생목록 재생 페이지 (자동 다음 영상, 영상 목록 사이드바, 이전/다음 버튼)

### 4.4 인증 프론트엔드
- [x] 로그인 페이지 (회원가입 링크)
- [x] 회원가입 페이지
- [x] useAuth 훅 (로그인 상태 관리, 토큰 갱신, 로그아웃)
- [x] 인증 가드 (미로그인 시 로그인 페이지로 리다이렉트)
